classdef Scheduler_Linprog_NoFlywheel
    %% Assumptions:
    ...
    methods
        function self = Scheduler_Linprog_NoFlywheel(renewable, battery, flywheel, demand, numOfIntervals, battery_rate_interval, fw_self_discharge_interval)
            num_of_inequality = 12 * numOfIntervals;
            num_of_equality = 4 * numOfIntervals;
        ...
        function self = setEqualityConstraints(self)
            ...
            % 1 * max_capacity
            for i = 1 : self.numOfIntervals
                self.A_eq(i + 2 * self.numOfIntervals, 9 * self.numOfIntervals + 1 + i - 1) = self.flywheel.max_capacity;

            end

            % Set D_f(i) to 0
            for i = 1 : self.numOfIntervals
                self.A_eq(i + 3 * self.numOfIntervals, 6 * self.numOfIntervals + 1 + i - 1) = 1;
                self.b_eq(i + 3 * self.numOfIntervals, :) = 0;
            end
...
%             for i = 2: size(battery_discharge_time, 1)
            ...
            self.flywheel_amortized_cost = 0;
